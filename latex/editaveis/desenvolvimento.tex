...
% ============================================================
% SEÇÕES A ESCREVER (diretriz 13/07: o texto descreve o MVP
% completo, como aplicativo pronto e recém-publicado nas lojas).
% Escrever em sessão dedicada E estender o parágrafo de
% organização do capítulo (acima) para anunciá-las.
%
% Prontidão do insumo (auditoria 13/07):
% - 6.5 Motor do fut: PRONTO para escrever (backend do match na
%   main; regras estáveis no fut_in_app §3-§6, §9).
%   Atenção: mínimo p/ iniciar = tamanho do time x 2 (derivado).
% - 6.6 Tempo real: PRONTO para escrever (gateway WS, cache/ETag,
%   seq e cron de backstop implementados; fut_in_app §8).
% - 6.7 Qualidade: PRONTO (CI nos dois repos, ~670 testes unit +
%   57 e2e no backend, jest-expo no front); ganha força se os
%   cenários A-M virarem suíte nomeada antes.
% - 6.8 Decisões/ADRs: PRONTO (candidatas: fila nunca
%   resequenciada, snapshot de regras por partida, WebSocket no
%   MVP, transições lazy + cron com fonte única).
% - Depende de implementação ainda: card pós-fut e estatísticas
%   (§6.1/§6.2), ativação de gerenciamento (§5.3), coleta passiva
%   (§5.2), SES real, telas do fut no aplicativo.
%
...
% \section{Decisões e trade-offs}  % 6.8 - 3-5 ADRs resumidas.
% ============================================================
